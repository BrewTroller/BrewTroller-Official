...
menu::menu(byte pageSize, byte maxOpts) {
    ...
    memset(_menuItems, 0, _maxOpts * sizeof(menuItem));
}
...
void menu::setItem(char disp[], byte value) {
    ...
    strncpy(_menuItems[index].name, disp, MAX_MENU_ITEM_NAME_LEN);
...
void menu::setItem_P(const char *disp, byte value) {
    byte index = this->getIndexByValue(value);
    if (index >= _maxOpts) return;
    strncpy_P(_menuItems[index].name, disp, MAX_MENU_ITEM_NAME_LEN);
    _menuItems[index].value = value;
    if (index == _itemCount) _itemCount++;
}
...
void menu::appendItem(const char* disp, byte value) {
    ...
    strncat(_menuItems[index].name, disp, MAX_MENU_ITEM_NAME_LEN - strnlen(_menuItems[index].name,MAX_MENU_ITEM_NAME_LEN));
}

void menu::appendItem_P(const char *disp, byte value) {
    byte index = this->getIndexByValue(value);
    if (index == _itemCount) return;
    strncat_P(_menuItems[index].name, disp, MAX_MENU_ITEM_NAME_LEN - strnlen(_menuItems[index].name, MAX_MENU_ITEM_NAME_LEN));
}
